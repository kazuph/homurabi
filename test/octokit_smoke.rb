...
OctokitSmoke.assert("token: option sets Authorization: Bearer ...") do
...
OctokitSmoke.assert("403 surface carries rate-limit response headers") do
  c = MiniOctokit.new
  raised = nil
  begin
...
OctokitSmoke.assert("retry middleware retries 503 then returns 200 body") do
  c = MiniOctokit.new
  u = c.user_req("kazuph").__await__.body
