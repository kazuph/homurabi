...
get "/test/crypto" do
  ...
  run.call("AES-256-CTR streaming") do
    ...
    plain = "streaming-" * 30
    e = OpenSSL::Cipher.new("AES-256-CTR").encrypt
    e.key = key
    e.iv = iv
    ct = ""
    i = 0
  ...
  run.call("AES-128-CBC round-trip") do
    ...
    iv = SecureRandom.random_bytes(16)
    e = OpenSSL::Cipher.new("AES-128-CBC").encrypt
    e.key = key
    e.iv = iv
    e.update("cbc-test")
    ct = e.final
  ...
  run.call("JWT RS256 encode/decode round-trip") do
  ...
  run.call("JWT ES256 encode/decode round-trip") do
  ...
  ed = OpenSSL::PKey::Ed25519.generate
  run.call("JWT EdDSA encode/decode round-trip") do
...
post "/api/enqueue" do
  ...
  begin
    ...
    body = {
      "note" => "default payload (empty or invalid JSON body)",
      "ts" => Time.now.to_i
    }
  end
  q.send(body)
...
get "/test/bindings" do
  ...
  begin
    ...
    if got.nil?
      ...
      cache_case[
        "note"
      ] = "match returned nil after put (cache unavailable in this runtime?)"
    else
...
get "/test/foundations" do
  ...
  run.call("Multipart parser extracts file + text field") do
  ...
  run.call("UploadedFile#to_uint8_array preserves raw bytes") do
  ...
  run.call("SSEStream frames data correctly") do
...
get "/docs/migration" do
  ...
  @docs_page = "migration"
  @docs_section = :guides
  @docs_breadcrumb = [%w[Docs /docs], ["Migration Guide", nil]]
  @docs_toc = [
    %w[scope 対象と非対象],
    %w[steps 6 ステップ],
    %w[gotchas よくあるハマり],
