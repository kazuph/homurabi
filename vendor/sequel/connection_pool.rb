...
class Sequel::ConnectionPool
  ...
  module ClassMethods
    ...
    # variants are deliberately not loaded because Mutex/Thread
    # aren't usable on Workers. Force single-threaded variants only.
    def connection_pool_class(opts)
      if pc = opts[:pool_class]
        unless pc.is_a?(Class)
          unless name = POOL_CLASS_MAP[pc]
            raise Sequel::Error, "unsupported connection pool type #{pc.inspect}, please pass appropriate class as the :pool_class option"
          end
        ...
        pc = opts[:servers] ? :sharded_single : :single
        connection_pool_class(:pool_class=>pc)
      end
    end
  end
