          ...
          when :next, :redo
            ...
            if !thrower_closure
              ...
              if thrower_closure.is? LOOP_INSIDE
                ...
                # and silently dropped the expression, so any assignment was
                # lost. Emit the expression as a side-effecting statement first.
                if value && !value.nil? && (!value.respond_to?(:type) || value.type != :nil)
                  push expr(value), '; '
                end
                push 'continue'
