...
| Example | 何を示すか |
|---|---|
| [`sinatra`](examples/sinatra/) | classic Sinatra README そのまま — `require 'sinatra'` + `get '/frank-says'`。`app.rb` 1 ファイル、D1 なし、views なし。 |
| [`classic-top-sinatra`](examples/classic-top-sinatra/) | `sinatra` と同じ形で `content_type :json` + JSON ルートを足し、classic トップレベル DSL を dogfood するもの。 |
| [`sinatra-with-db`](examples/sinatra-with-db/) | 最小の D1 付き Sinatra: `Sequel.connect(adapter: :d1, d1: env['cloudflare.DB'])`、ルート 1 個、マイグレーション 1 個。 |
| [`sinatra-with-email`](examples/sinatra-with-email/) | Phase 17.5 auto-await デモ — `SEND_EMAIL` Cloudflare Email バインディング越しの POST `/send`、ソース上に `.__await__` ゼロ。 |
| [`todo-simple`](examples/todo-simple/) | **最小のステートフルサンプル。** `app.rb` 1 つ、`views/` なし、D1 なし — HTML は Ruby の heredoc で書く。「homura はどれだけ少なくて済むか」を示す。 |
| [`todo`](examples/todo/) | ORM なしの D1 ベース CRUD — `env['cloudflare.DB']` と `Cloudflare::D1Database` を直接使う。 |
| [`todo-orm`](examples/todo-orm/) | 同じ TODO アプリを `sequel-d1` 経由で書いたもの。マイグレーション、データセットチェイン、`.first` / `.update`。 |
| [`auth-otp`](examples/auth-otp/) | メール OTP ログイン。開発時は [mailpit](https://mailpit.axllent.org/) で送信、HMAC 署名付きセッション cookie、`rake e2e:headed` でフルの headed Playwright E2E。 |
| [`blog`](examples/blog/) | 小さなブログ: 一覧 / 詳細 / 新規 / **適切な 404** / 削除。非同期ルートのステータス保持と `<%= h(post[:body]).gsub("\n", "<br>") %>` を示す。 |
| [`inertia-todo`](examples/inertia-todo/) | [Inertia.js](https://inertiajs.com) + Vue 3 による薄い SPA。Sinatra がページ props を提供する。クライアント JS は `public/assets/` 配下。 |
| [`hotwire-todo`](examples/hotwire-todo/) | Turbo Streams（Accept ネゴシエーション越しのサーバーレンダリング partial）+ オートフォーカス用の小さな Stimulus コントローラー。 |
