...
def make_app(
  ...
  Class.new(Sinatra::Base) do
    # Sinatra 4.x: disable host check for rack-test
    ...
    if File.directory?(File.join(__dir__, "test", "views"))
      set(:views, File.join(__dir__, "test", "views"))
    end
...
File.write(
  File.join(LAYOUT_VIEWS, "layout.erb"),
  <<~ERB
    <!doctype html>
    <html><body>
...
class InertiaProtocolTest < Minitest::Test
  ...
  def test_initial_get_returns_html_with_data_page
    a = make_app { get("/") { inertia("Hello", props: {name: "world"}) } }
    with(a) do
      get("/")
  ...
  def test_inertia_visit_returns_json
    ...
    with(a) do
      ...
      assert_equal(200, last_response.status)
      assert_includes(
        last_response.headers["Content-Type"].to_s,
        "application/json"
      )
  ...
  def test_version_mismatch_returns_409_with_location
    a = make_app(version: "v2") { get("/") { inertia("Hello", props: {}) } }
    with(a) do
      header("X-Inertia", "true")
  ...
  def test_partial_reload_includes_only_requested_props
    a = make_app do
      get("/") do
        inertia("Page", props: {a: -> { "A" }, b: -> { "B" }, c: -> { "C" }})
      end
    end
  ...
  def test_defer_prop_resolved_on_partial_request
    a = make_app do
      get("/") { inertia("Page", props: {stats: Inertia.defer { "STATS" }}) }
    end
    ...
    with(a) do
      ...
      refute(
        page.key?("deferredProps"),
        "should not list resolved deferred props"
      )
    end
  end
  ...
  def test_optional_prop_only_on_partial_request
    a = make_app do
      get("/") { inertia("Page", props: {filter: Inertia.optional { "F" }}) }
    end
    ...
    with(a) do
      ...
      page = JSON.parse(last_response.body)
      refute(
        page["props"].key?("filter"),
        "optional prop omitted on initial visit"
      )
  ...
  def test_merge_prop_listed_in_mergeProps
    a = make_app do
      get("/") { inertia("Feed", props: {items: Inertia.merge([1, 2, 3])}) }
    end
  ...
  def test_render_inertia_alias_works_like_inertia_helper
    a = make_app { get("/legacy") { render(inertia: "Legacy", props: {x: 1}) } }
    with(a) do
      header("X-Inertia", "true")
      header("X-Inertia-Version", "1")
      get("/legacy")
  ...
  def test_errors_session_sweeps_after_one_render
    ...
    with(a) do
      ...
      page = JSON.parse(last_response.body)
      refute(
        page["props"].key?("errors"),
        "errors should be swept after first render"
      )
    end
  end
  ...
  def test_303_promotion_for_non_get_redirects
    a = make_app do
      # 302 by default → middleware promotes to 303
      put("/x") { redirect("/y") }
  ...
  def test_encrypt_history_flag
    a = make_app(encrypt_history: true) { get("/") { inertia("Page", props: {}) } }
    with(a) do
      header("X-Inertia", "true")
      header("X-Inertia-Version", "1")
      get("/")
  ...
  def test_once_constructor_is_not_part_of_public_api
    refute(
      Sinatra::Inertia.respond_to?(:once),
  ...
  def test_csrf_can_be_opted_out
    a = make_app(csrf: false) { post("/save") { "ok" } }
    with(a) do
      post("/save")
      assert_equal(200, last_response.status)
    end
  ...
  def test_csrf_helper_returns_current_token
    a = make_app(csrf: true) { get("/token") { csrf_token.to_s } }
    with(a) do
      get("/token")
      assert_equal(200, last_response.status)
      cookie = last_response.headers["Set-Cookie"].to_s
