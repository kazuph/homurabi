...
module Sinatra
  ...
  # homurabi: raw header presence check — the Cloudflare runtime
  ...
  # CRuby but not in homurabi's vendored Rack. The public Iterator
  ...
  module Helpers
    ...
    # homurabi port switched to `, ` between key=value params to match
  ...
  # with a CGI-backed stand-in), so this works today. The homurabi
  ...
  class Base
    ...
    # homurabi adds a fourth case: a native JS Promise (routes with
    ...
    class << self
      ...
      # homurabi does not use host_authorization on Workers (the
      ...
      def setup_default_middleware(builder)
        ...
        # (would require IPAddr, also stubbed in homurabi). Skipping it
...
# in homurabi, so leaving them set is inert. Clearing static_headers
