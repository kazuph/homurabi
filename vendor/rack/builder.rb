...
module Rack
  ...
  class Builder
    ...
    def use(middleware, *args, &block)
      ...
      # homurabi debug: log every middleware wrap with the inner app class.
      @use << proc { |app|
        inner_class = app.class.name rescue 'unknown'
        mw_name = middleware.respond_to?(:name) ? middleware.name : middleware.to_s
        `console.error('[HOMURABI use-proc] mw=' + #{mw_name} + ' inner_app=' + #{inner_class} + ' (app nil? ' + (#{app}.$$is_nil ? 'YES' : 'no') + ')')`
    ...
    def to_app
      `console.error('[HOMURABI to_app] @run class=' + (#{@run} && #{@run}.$$class && #{@run}.$$class.$$name) + ' @run nil?=' + (#{@run} === nil || (#{@run} && #{@run}.$$is_nil)))`
      app = @map ? generate_map(@run, @map) : @run
      `console.error('[HOMURABI to_app] after assignment, app class=' + (#{app} && #{app}.$$class && #{app}.$$class.$$name))`
