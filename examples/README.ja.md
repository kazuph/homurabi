...
| [`inertia-todo/`](inertia-todo/) | Sinatra + Inertia.js + Vue 3 | <https://inertia-todo.kazu-san.workers.dev/> | サーバーレンダリングされる Inertia ページオブジェクト、`X-Inertia` コンテンツネゴシエーション、JSON props。クライアント JS は `public/assets/inertia-app.js` に存在。 |
| [`hotwire-todo/`](hotwire-todo/) | Sinatra + Turbo Streams + Stimulus | <https://hotwire-todo.kazu-san.workers.dev/> | `Accept: text/vnd.turbo-stream.html` ネゴシエーション越しのサーバーレンダリング turbo-stream パーシャル。クライアント JS はオートフォーカス用の小さな Stimulus コントローラーのみ。 |
