...
class InertiaProtocolTest < Minitest::Test
  ...
  def test_inertia_visit_returns_json
    ...
    with(a) do
      ...
      get "/"
      assert_equal 200, last_response.status
      assert_includes last_response.headers["Content-Type"].to_s,
                      "application/json"
  ...
  def test_inertia_share_merges_into_every_response
    a =
      make_app(share: -> { { auth: { user: { id: 7, name: "Alice" } } } }) do
  ...
  def test_render_inertia_alias_works_like_inertia_helper
    a =
      make_app { get("/legacy") { render inertia: "Legacy", props: { x: 1 } } }
    with(a) do
      header "X-Inertia", "true"
      header "X-Inertia-Version", "1"
      get "/legacy"
  ...
  def test_encrypt_history_flag
    a =
      make_app(encrypt_history: true) { get("/") { inertia "Page", props: {} } }
    with(a) do
      header "X-Inertia", "true"
      header "X-Inertia-Version", "1"
      get "/"
  ...
  def test_encrypt_history_flag_per_route_override
    a =
      make_app(encrypt_history: false) do
        get("/") do
