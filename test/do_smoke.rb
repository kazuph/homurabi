...
SmokeTest.assert(
  "new_unique_id forwards to js.newUniqueId and returns a DurableObjectId"
) do
  js_ns = fake_namespace
  ns = Cloudflare::DurableObjectNamespace.new(js_ns)
  a = ns.new_unique_id
...
SmokeTest.assert("get_by_name composes id_from_name + get") do
  ...
  stub.is_a?(Cloudflare::DurableObjectStub) &&
    `#{js_ns}._calls.idFromName[0]` == "demo"
end
...
SmokeTest.assert(
  "dispatcher calls the registered handler and returns a JS Response"
) do
...
SmokeTest.assert(
  "handler return-value normaliser accepts String as 200 text/plain"
) do
...
SmokeTest.assert(
  "block_concurrency_while falls back to the raw promise when state lacks BCW"
) do
...
SmokeTest.assert(
  "block_concurrency_while serialises two reads of a shared counter"
) do
...
SmokeTest.assert(
  "define_web_socket_handlers registers on_message + on_close handlers"
) do
...
SmokeTest.assert(
  "WS message dispatcher invokes the registered on_message lambda"
) do
  received = {}
  Cloudflare::DurableObject.define_web_socket_handlers(
    "WsMsgSmoke",
  ...
  received[:msg] == "hello-inside" &&
    received[:state].is_a?(Cloudflare::DurableObjectState) &&
    received[:state].id == "ws-id"
end

SmokeTest.assert(
  "WS close dispatcher invokes the registered on_close lambda with code + reason"
) do
  captured = nil
  Cloudflare::DurableObject.define_web_socket_handlers(
    "WsCloseSmoke",
...
SmokeTest.assert(
  "WS dispatch for an unregistered class is a no-op, returns nil"
) do
