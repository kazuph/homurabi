...
#
# A single-file classic Sinatra app: top-level `get '/' do ... end`,
# no `Sinatra::Base` subclass, no `views/` directory, no D1, no
# migrations. The HTML is built from heredocs right next to the
# routes, and the data lives in a top-level Ruby array. The Worker
# isolate's lifetime IS the data's lifetime — restart wrangler dev
# and your todos are gone. That's the price of "no setup".
#
