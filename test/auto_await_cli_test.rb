...
Dir.mktmpdir do |dir|
  ...
  File.write(
    File.join(lib_dir, "homura_async_sources.rb"),
    <<~RUBY
  ...
  File.write(
    File.join(input_dir, "manual_await.rb"),
    <<~RUBY
      module Demo
        def self.fetch
  ...
  File.write(
    File.join(input_dir, "sequel_chain.rb"),
    <<~RUBY
      require 'sequel'
  ...
  File.write(
    File.join(input_dir, "helper_propagation.rb"),
    <<~RUBY
      def load_rows
        kv.get('todos')
      end
