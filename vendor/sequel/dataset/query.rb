...
module Sequel
  class Dataset
    ...
    UNCONDITIONED_JOIN_TYPES.each do |jtype|
      ...
      jtype_sym = jtype
      jtype_name = jtype.to_s
      define_method(:"#{jtype_name}_join") do |table, opts = Sequel::OPTS, &block|
        raise(Sequel::Error, "#{jtype_name}_join does not accept join table blocks") if block
        raise(Sequel::Error, "#{jtype_name}_join 2nd argument should be an options hash, not conditions") unless opts.is_a?(Hash)
        join_table(jtype_sym, table, nil, opts)
      end
    end
