...
Dir.mktmpdir do |dir|
  ...
  File.write(
    File.join(views_dir, "layout_docs.erb"),
    "<article><%= yield %></article>"
  )
  File.write(
    File.join(views_dir, "cvars.erb"),
    "<ul>\n<% @@todos.each do |t| %>\n  <li><%= t %></li>\n<% end %>\n</ul>\n<p><%= @@todos.length %></p>"
  )
  File.write(
    File.join(views_dir, "cvars_assign.erb"),
    "<% @@count = @@count + 1 %><span><%= @@count %></span>"
  )
  File.write(
    File.join(load_path_dir, "sinatra", "base.rb"),
    "module Sinatra\n  module Templates\n  end\nend\n"
  )
...

  {
    "compile-erb rejects <% yield %>" => {
      argv: [
        "ruby",
        File.expand_path("../gems/homura-runtime/exe/compile-erb", __dir__)
      ],
      body: "<% yield %>"
    },
    ...
    "homura erb:compile rejects <% yield %>" => {
      argv: [
        ...
        "exec",
        "ruby",
        File.expand_path("../gems/sinatra-homura/bin/homura", __dir__),
        "erb:compile"
      ],
      body: "<% yield %>"
    },
