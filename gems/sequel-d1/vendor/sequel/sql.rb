  ...
  module SQL
    ...
    class Expression
      ...
      class << self
        ...
        def to_s_method(meth, args=:self) # :nodoc:
          ...
          # reads at call time). We parse a small Ruby expression
          # grammar — comma-separated ivars, integer/string/symbol
          # literals, bare method names on self — and resolve at call
          # time. Sites Sequel upstream uses:
          #   :self          → [self]
          #   '@op, @args'   → [@op, @args]
          #   '@constant'    → [@constant]
          #   '@value'       → [@value]
          #   '@table, @column' → [@table, @column]
          # Original: class_eval("def to_s_append(ds, sql) ds.#{meth}_append(sql, #{args}) end", ...)
          append_meth = :"#{meth}_append"
          if args == :self
            define_method(:to_s_append) { |ds, sql| ds.public_send(append_meth, sql, self) }
          else
            resolvers = args.to_s.split(',').map { |expr| parse_to_s_method_arg(expr.strip) }
            define_method(:to_s_append) do |ds, sql|
              resolved = resolvers.map { |r| r.call(self) }
              ds.public_send(append_meth, sql, *resolved)
            end
          end
          @comparison_attrs.freeze
        end
        ...
        # else raises so we notice future Sequel upstream additions.
        def parse_to_s_method_arg(expr)
          case expr
          when /\A@\w+\z/
            iv = expr.to_sym
            ->(instance) { instance.instance_variable_get(iv) }
          when /\A-?\d+\z/
            value = expr.to_i
            ->(_instance) { value }
          when /\A'([^']*)'\z/
    ...
    module BooleanMethods
      ComplexExpression::BOOLEAN_OPERATOR_METHODS.each do |m, o|
        ...
        meth_sym = m
        op_sym = o
        define_method(meth_sym) do |other|
          case other
          when NumericExpression, Integer
            NumericExpression.new(meth_sym, self, other)
          else
            BooleanExpression.new(op_sym, self, other)
          end
        end
      end
    ...
    module OperatorBuilders
      ...
      # body time would fail. We cache the resolved class on first call.
      {'NumericExpression'=>{'+'=>'+', '-'=>'-', '*'=>'*', '/'=>'/'},
       'BooleanExpression'=>{'&'=>'AND', '|'=>'OR'}}.each do |klass_name, ops|
        ops.each do |m, op|
          meth_sym = m.to_sym
          op_sym = op.to_sym
          captured_klass_name = klass_name
          define_method(meth_sym) do |*args|
            klass_ref = ::Sequel::SQL.const_get(captured_klass_name)
            if args.length == 1
              v = args.first
              if v.class.is_a?(klass_ref)
                v
              else
                klass_ref.new(:NOOP, v)
              end
            else
              klass_ref.new(op_sym, *args)
            end
          end
        end
      end
