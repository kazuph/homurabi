...
row_conversion_cases = [
  ...
  },
  {
    ...
    pairs: [
      ["id", 13],
      %w[due_date __HOMURA_SENTINEL_NULL__],
  ...
  },
  {
    ...
    pairs: [
      ["id", 14],
      %w[archived_at __HOMURA_SENTINEL_UNDEFINED__],
      ["priority", 3]
    ],
    nil_key: "archived_at",
    preserved: {
...
SequelSmoke.assert_matrix(
  "js_object_to_hash matrix",
  row_conversion_cases
) do |test_case|
...
mixed_row_cases = [
  ...
  },
  {
    ...
    rows: [
      [["id", 23], %w[archived_at __HOMURA_SENTINEL_NULL__], ["priority", 0]],
  ...
  },
  {
    ...
    rows: [
      [
        ["id", 25],
        %w[due_date __HOMURA_SENTINEL_UNDEFINED__],
        %w[archived_at __HOMURA_SENTINEL_NULL__],
        ["created_at", 1_700_000_400]
      ],
      [
        ["id", 26],
        ["due_date", 1_735_689_602],
        %w[archived_at __HOMURA_SENTINEL_NULL__],
        ["completed", false],
        ["priority", 0]
      ]
...
SequelSmoke.assert_matrix(
  "js_rows_to_ruby matrix",
  mixed_row_cases
) do |test_case|
  js_rows = `[]`
  test_case[:rows].each { |pairs| `#{js_rows}.push(#{build_js_row(pairs)})` }
...
SequelSmoke.assert_equal(
  ...
  db[:users].where(id: 1).update_sql(
    created_at: ::Sequel.function(:CURRENT_TIMESTAMP)
  )
)
