...
SmokeTest.assert("send_batch accepts a list of plain bodies") do
  js_p = fake_queue_producer
  q = Cloudflare::Queue.new(js_p, "jobs")
  q.send_batch(%w[a b c]).__await__
...
SmokeTest.assert(
  "send_batch with delay_seconds forwards top-level opts.delaySeconds"
) do
  js_p = fake_queue_producer
  q = Cloudflare::Queue.new(js_p, "jobs")
  q.send_batch(%w[a b c], delay_seconds: 17).__await__
...
SmokeTest.assert(
  "QueueMessage#retry with delay_seconds forwards options.delaySeconds"
) do
  batch = fake_batch("jobs", [`({ v: 1 })`])
  msgs = Cloudflare::QueueBatch.new(batch).messages
...
SmokeTest.assert(
  "QueueBatch#retry_all with delay_seconds forwards options.delaySeconds"
) do
  batch = fake_batch("jobs", [`({ a: 1 })`])
