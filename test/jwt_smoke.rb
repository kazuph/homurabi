...
SmokeTest.assert("RS256 accepts private key for verification (CRuby compat)") do
...
SmokeTest.assert("ES256 rejects ES384 key (curve/alg mismatch)") do
...
SmokeTest.assert(
  "decode with algorithm mismatch is rejected (HS256 token, RS256 expected)"
) do
...
SmokeTest.assert("decode without verify returns segments unchecked") do
  token = JWT.encode(PAYLOAD, "secret", "HS256").__await__
...
SmokeTest.assert(
  "decode raises JWT::DecodeError on malformed input (2 segments)"
) do
  raised = false
  begin
