...
build_cli =
  File.join(repo_root, "gems", "homura-runtime", "exe", "homura-build")
bundle_env = { "BUNDLE_GEMFILE" => File.join(repo_root, "Gemfile") }

ok =
  assert("homura CLI shows new subcommand usage") do
...
Dir.mktmpdir do |dir|
  ...
  ok =
    assert("homura new scaffolds a project") do
...
Dir.mktmpdir do |dir|
  ...
  ok =
    assert("homura new --with-db scaffolds build task with db flag") do
      Dir.chdir(repo_root) do
...
Dir.mktmpdir do |dir|
  ...
  ok =
    assert(
      ...
      entrypoint =
        File.read(File.join(app_dir, "build", "worker.entrypoint.mjs"))
...
Dir.mktmpdir do |dir|
  ...
  ok =
    assert("standalone build falls back to app/app.rb without app/hello.rb") do
      output, status =
        Open3.capture2e(
          bundle_env,
          "bundle",
...
Dir.mktmpdir do |dir|
  ...
  ok =
    assert("standalone build preserves route params across await") do
      output, status =
        Open3.capture2e(
          bundle_env,
          "bundle",
...
Dir.mktmpdir do |dir|
  ...
  ok =
    ...
    JS
      ...
      unless api_line ==
               'API:200:{"todos":[{"id":1,"text":"demo","completed":false}]}'
        raise output
      end
