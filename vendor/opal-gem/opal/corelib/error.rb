...
# throw/catch as plain raise/rescue (kernel.rb:824-834), which means an
# in-flight throw collides with any `rescue StandardError` the caller
# happens to have on the stack. Sinatra's `process_route` is the most
# visible real-world victim: its `rescue StandardError; raise` wrapper
# unconditionally eats `throw :halt` before it can reach the `catch(:halt)`
# that `invoke` installs.
#
# Changing the base class to `::Exception` preserves the rescuability of
# an *uncaught* throw (users can still `rescue UncaughtThrowError`) while
# preventing routine `rescue StandardError` frames on the call stack from
# accidentally swallowing mid-flight throws. This matches the externally
# observable MRI behaviour for every real-world use of throw/catch.
class ::UncaughtThrowError    < ::Exception; end
