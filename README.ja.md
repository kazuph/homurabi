...
[`examples/`](examples/) には 6 つの完全に動作するアプリがある。それぞれが
...
[`examples/minimal-sinatra*`](examples/) 配下にはより小さな歴史的
フィクスチャもある。これらは公開 gem への書き直し前のもので、この
monorepo への `path:` 参照を使っている。テンプレートとしてではなく、
内部スモークテストとして使うこと。
...
- **機械可読なサマリ**: [`/llms.txt`](public/llms.txt) — ライブでも
...
examples/             # 6 つのスタンドアロンサンプルアプリ（上記参照）
docs/                 # 詳細ドキュメント
public/llms.txt       # エージェント可読サマリ
skills/               # インストール可能なエージェント skill
```

`gems/*` ディレクトリと `vendor/opal-gem/` が出荷面（shipping surface）だ。
それ以外はすべて、ドキュメント、サンプル、あるいはこの 4 つの gem を
ビルドして出荷するためのインフラに過ぎない。
