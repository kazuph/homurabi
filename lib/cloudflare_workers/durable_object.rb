...
#
# The JS side (src/worker.mjs) exports a generic `HomurabiCounterDO`
# class whose constructor + fetch forward to Ruby through
# `globalThis.__HOMURABI_DO_DISPATCH__(class_name, state, env, req_init)`.
...
module Cloudflare
  ...
  #
  # The JS side (src/worker.mjs) defines a single `HomurabiCounterDO`
  # export. When the DO runtime instantiates it and calls its fetch,
  # the JS class hands the call off to
  # `globalThis.__HOMURABI_DO_DISPATCH__(class_name, state, env, request, body_text)`.
