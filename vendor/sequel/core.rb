...
# We swallow LoadError ONLY for the Opal-non-shipped set, so that
...
module Sequel
  ...
  module SequelMethods
    ...
    # "sequel/database.rb/../database/connecting" from its join of
    # __FILE__ and the relative target; we normalise these by walking
    # segments and collapsing `..` pairs, so the final path we hand
    # to `Kernel.require` matches what other modules look up
    # (`sequel/database/connecting`).
    #
    # Upstream Sequel called `Kernel.require_relative` directly with
    # an interpolated string — Opal's static-require analyser hit an
    # infinite recursion on that pattern because it could not resolve
    # the variable component at compile time.
  ...
  # Sequel via `extend SQL::Builders` (sql.rb:2080). Opal's
  # `alias_method` on the singleton class does not see methods
  # inherited through `extend`, so the alias fires
  # `NameError: undefined method \`expr'`. Install via
  # `define_singleton_method` which forwards at call time instead,
  # matching the CRuby behaviour.
  def self.[](*args, &block)
    expr(*args, &block)
  end
  ...
  def_adapter_method(*::Sequel::Database::ADAPTERS)
end
