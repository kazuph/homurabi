...
Phase 13 で janbiedermann fork を離脱し、上流 Sinatra 4.x に揃ったことで、homurabi が
...
- gem 化容易性スコア **25-30/100** — 内核は意外と clean、外殻が homurabi 専用に焼かれてる
- `app/hello.rb` 2,814 行に library 的ヘルパと demo route が混在（境界が見えてない）
- `package.json` / `bin/*` が `app/hello.rb`・`views/`・`homurabi_templates` を hardcode
- `src/worker.mjs` が `__HOMURABI_*` グローバル名で固有化（14 箇所）
- `lib/sequel/adapters/d1.rb:35` が `require 'cloudflare_workers'` 直結（要 hook 化）
- `vendor/sequel/*` に inline `# homurabi patch:` コメント付きの class_eval→define_method
...
gem 'sinatra'
gem 'sinatra-cloudflare-workers'  # ← これが核
...
- **3 gem に絞る**（v1 の 5 gem 案から降格）。`jwt` / `http` shim は homurabi 内に温存し、
  境界が立ってから将来 gem 化（Phase 15-Future として記録）
- **Sinatra/Sequel は完全 opt-in**。`cloudflare-workers-runtime` (core) は Sinatra も
  Sequel も知らない
- **vendor 改変は禁止方向**。すべて外部 monkey patch (`require` 後に `prepend` /
  `class_reopen`) に統一。vendor 改変が残るのは「上流 PR で吸収するまでの暫定」のみで、
  必ず TODO 化
- **homurabi リポジトリは「showcase アプリ」として残す** — gem を `path:` 指定で依存し、
...
| Phase 15-B 完了 | 55 | `cloudflare-workers-runtime` gem 切り出し（コア層） |
| Phase 15-C 完了 | 80 | `sinatra-cloudflare-workers` gem 切り出し（本命・ユーザー接点）+ examples/ + 移植 dogfood |
...
1. **Opal stdlib 削減**
   - `vendor/opal-gem/stdlib/` から homurabi 未使用のものを排除
...
1. **`src/worker.mjs` の `__HOMURABI_*` 名前空間化**
   - `__HOMURABI_RACK_DISPATCH__` → `globalThis.__OPAL_WORKERS__.rack` のような namespace 化
   - DO / Queue / Scheduled / WS handler 全部同様に
   - homurabi 側からは `register_dispatcher(:rack, fn)` のような hook で登録
   - **DoD**: `src/worker.mjs` 内に `HOMURABI` 文字列ゼロ
2. **build script の引数化**
   - `bin/compile-erb` に `--input` `--output` `--namespace` argv 追加
   - `bin/compile-assets` に同様
   - `bin/patch-opal-evals.mjs` を generic CLI 化
   - homurabi 側は `Rakefile` 経由で呼ぶ:
     ```ruby
     namespace :build do
       task :erb do
         sh 'bin/compile-erb --input views --output build/homurabi_templates.rb ' \
...
1. **monorepo 内に `gems/cloudflare-workers-runtime/` 作成**
   ```
   gems/cloudflare-workers-runtime/
     cloudflare-workers-runtime.gemspec
   ...
   ```
   - homurabi の `Gemfile`: `gem 'cloudflare-workers-runtime', path: 'gems/cloudflare-workers-runtime'`
...
4. **homurabi 自身が新 gem を path 依存で動く**
...
### Phase 15-C — `sinatra-cloudflare-workers` gem 切り出し（本命・ユーザー接点）
...
2. **monorepo 内に `gems/sinatra-cloudflare-workers/` 作成**
   ```
   gems/sinatra-cloudflare-workers/
     sinatra-cloudflare-workers.gemspec
     ...
     README.md
   ```
   - 依存: `cloudflare-workers-runtime` (15-B)、`sinatra` ~> 4.2、`mustermann`、`rack` ~> 3
...
8. **既存 homurabi が新 gem を path 依存で動く**
   - 全 smoke 緑
...
| 自己 dogfood の小アプリ移植で想定外バグ | 移植時 issue は homurabi で fix → gem に反映 |
...
1. **monorepo 内に `gems/sequel-d1/` 作成**
   ...
   gems/sequel-d1/
     ...
     bin/cloudflare-workers-migrate         # bin/homurabi-migrate を generic 化
     README.md
   ```
   - **vendor/sequel/ の inline 改変は事前に上流 PR or lib 側 monkey patch に移行済前提**
     （Phase 15-A の vendor 棚卸しと連動）
   - 依存: `sequel` ~> 5.x、`cloudflare-workers-runtime`
...
- `cloudflare-workers-jwt` 候補
  ...
  - 当面: `vendor/jwt/` は homurabi に残し、`lib/sinatra/jwt_auth.rb` は
    `sinatra-cloudflare-workers` gem (15-C) に同梱
...
- `cloudflare-workers-http` 候補
  ...
  - **問題**: shim は cloudflare-workers-runtime の `Cloudflare::HTTP` に強く依存し、
    実質「core gem の拡張」。独立 gem の意義が薄い
  - 当面: `vendor/faraday.rb` `vendor/net/` は `cloudflare-workers-runtime` (15-B) に
    同梱（または `cloudflare-workers-runtime-net-http` 互換 shim として core 内に置く）
...
1. **Gem 構成（最終形）**: **3 gem**（v1 の 5 gem から降格）
   - `cloudflare-workers-runtime` — コア（必須・Sinatra も Sequel も知らない）
   - `sinatra-cloudflare-workers` — Sinatra ユーザー向け（最も入口になる gem、JWT 拡張も同梱）
   - `sequel-d1` — DB 使う人向け（opt-in）

2. **homurabi リポジトリ自身が「showcase アプリ」として動く**
   - 3 つの gem を `path:` 依存で参照
   - 既存 全 smoke + 全 dogfood ルートが緑
   - README が「homurabi を使う」ではなく「**この技術スタックを自分のアプリに導入する**」誘導に書き換わる
...
Phase 15-A (toolchain 契約抽出・最小粒度)   ← 1 週間
  └─ DoD: worker.mjs から HOMURABI 文字列ゼロ、build CLI 引数化、
        VENDOR_PATCHES.md 完成、TOOLCHAIN_CONTRACT.md 完成
       ↓
Phase 15-B (cloudflare-workers-runtime gem) ← 1 週間
  └─ DoD: 3 gem 中 1 つ完成、homurabi が path 依存で動く
       ↓
Phase 15-C (sinatra-cloudflare-workers gem) ← 2 週間  ★本命
