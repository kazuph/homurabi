...
module Sequel
  class Dataset
    ...
    def self.def_sql_method(mod, type, clauses)
      ...
      # (simple clause list OR clause pairs with if/elsif/else
      # fragments — the latter is used by e.g. the :select shape).
      priv = (type == :update || type == :insert)
      cacheable = (type == :select || type == :delete)
      meth_name = :"#{priv ? '_' : ''}#{type}_sql"

      # Pre-compute the list-of-methods shape so the runtime block
      # doesn't do repeated work on every SQL generation.
      if clauses.all? { |c| c.is_a?(Array) }
        # Branched shape:
        #   [['if db.sqlite_version >= 33500', %w'with ... returning'],
        #    ['elsif db.sqlite_version >= 30803', %w'with ... delete'],
        #    ['else', %w'delete from where']]
        # Upstream class_eval'd the string conditions; we translate
        # every pattern across all built-in adapters into lambdas
        # evaluated against `self`.
        cond_methods_pairs = clauses.map do |branch_code, branch_clauses|
          meths = clause_methods(type, branch_clauses)
          [parse_def_sql_method_branch_condition(branch_code), meths]
        end
        branched = true
      else
        methods_to_call = clause_methods(type, clauses)
        branched = false
      end

      delete_checks = (type == :delete)

      mod.send(:define_method, meth_name) do
        unless priv
          if (pre_sql = opts[:sql])
            next static_sql(pre_sql)
          end
        end
        if cacheable
          if (cached = cache_get(:"_#{type}_sql"))
            next cached
          end
        end
        if delete_checks
          check_delete_allowed!
          check_not_limited!(:delete)
        end
        sql = @opts[:append_sql] || sql_string_origin
        if branched
          cond_methods_pairs.each do |cond, meths|
            if cond.call(self)
              meths.each { |m| public_send(m, sql) }
              break
            end
          end
        else
          methods_to_call.each { |m| public_send(m, sql) }
        end
        if cacheable
          cache_set(:"_#{type}_sql", sql) if cache_sql?
        end
        sql
      end
    ...
    #
    # Supported (exhaustive list audited against Sequel v5.103.0):
    #   if|elsif db.sqlite_version >= N
    #   if|elsif server_version >= N                 (postgres)
    #   if|elsif is_2005_or_later?                   (mssql)
    #   if|elsif opts[:values]
    #   else
    def self.parse_def_sql_method_branch_condition(branch_code)
      case branch_code
      when /\A(?:if|elsif) db\.sqlite_version >= (\d+)\z/
        min = Regexp.last_match(1).to_i
        ->(ds) { ds.db.sqlite_version >= min }
      when /\A(?:if|elsif) server_version >= (\d+)\z/
        min = Regexp.last_match(1).to_i
        ->(ds) { ds.server_version >= min }
      when /\A(?:if|elsif) (\w+\?)\z/
        predicate = Regexp.last_match(1).to_sym
        ->(ds) { ds.send(predicate) }
      when /\A(?:if|elsif) opts\[:(\w+)\]\z/
    ...
    [:literal, :quote_identifier, :quote_schema_table].each do |meth|
      ...
      define_method(meth_sym) do |*args, &block|
        s = ''.dup
        public_send(append_sym, s, *args, &block)
        s
      end
    end
