...
Dir.mktmpdir do |dir|
  ...
  ok =
    assert(
      "auto-await emits file when only manual __await__ needs magic comment"
    ) do
  ...
  ok =
    assert("auto-await still skips unchanged sync-only files") do
  ...
  ok =
    assert("auto-await loads sequel-d1 async sources for Dataset chains") do
      Dir.chdir(repo_root) do
        system(
          bundle_env,
          "bundle",
      ...
      unless output.include?(
               "seq_db[:users].where(active: true).order(:name).limit(10).all.__await__"
             )
        raise output
      end
      unless output.include?("seq_db[:users].where(id: 1).first.__await__")
        raise output
      end
    end
  passed += 1 if ok
  ...
  ok =
    assert("auto-await propagates async helper methods to call sites") do
      Dir.chdir(dir) do
        system(
          bundle_env,
          "bundle",
